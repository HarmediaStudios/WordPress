...
function wpfc_em_ajax() {
	...
	foreach ( $EM_Events as $EM_Event ) {
		...
		if($add_event && $event_date_counts[$event_date] <= $limit ){
			...
			$events[] = array ("title" => $title, "post_image" => $post_image, "color" => $color, 'textColor'=>$textColor, 'borderColor'=>$borderColor, "start" => date('Y-m-d\TH:i:s', $EM_Event->start), "end" => date('Y-m-d\TH:i:s', $EM_Event->end), "url" => $EM_Event->get_permalink(), 'post_id' => $EM_Event->post_id, 'event_id' => $EM_Event->event_id, 'allDay' => $EM_Event->event_all_day == true );
		}elseif( empty($event_dates_more[$event_date]) ){
			$event_dates_more[$event_date] = 1;
			$day_ending = $event_date."T23:59:59";
			$events[] = apply_filters('wpfc_events_more', array ("title" => get_option('wpfc_limit_txt','more ...'),  "post_image" => $post_image, "color" => get_option('wpfc_limit_color','#fbbe30'), "start" => $day_ending, "url" => str_replace('%s',$event_date,$event_page_link), 'post_id' => 0, 'event_id' => 0 ,'allDay' => true), $event_date);
		}
	}
