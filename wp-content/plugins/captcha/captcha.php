...
$cptch_admin_fields_enable = array (
		...
		array( 'cptch_anonymous_post', __( 'Anonymous Posting', 'captcha' ), __( 'Anonymous Posting', 'captcha' ) ),
);
...
// Add captcha into cptch_anonymous_post
if ( 1 == $cptch_options['cptch_anonymous_post'] ) {
	add_action( 'new_post', 'cptch_anonymous_post_form' );
} else {
	remove_action( 'new_post', 'cptch_anonymous_post_form');
}
...
// this function adds the captcha to the register form
function cptch_anonymous_post_form() {
	global $cptch_options;

	echo "<label for='text'>Security Verification</label> :(*)<br/>";
	if(function_exists('cptch_display_captcha_custom')) { 
		echo '<div style="margin-bottom: 10px;">';
			echo cptch_display_captcha_custom();
			echo "<input type='hidden' name='cntctfrm_contact_action' id='cntctfrm_contact_action' value='true'/>";
		echo '</div>';
	}
}

// this function checks captcha posted with registration
function cptch_anonymous_post_submit() {
	global $str_key;
	$str_key = "bws18042013";
